...
int connect_to_runtime_peer(unsigned int id, struct sockaddr_in *addr){
    ...
    int rtn = send_to_peer(id, sizeof(hdr), &hdr);
    if (rtn < 0) {
        log_error("Could not send header for initial connection to peer runtime %d", id);
        return -1;
    }
    rtn = send_to_peer(id, sizeof(msg), &msg);
    if (rtn < 0) {
