...
int init_thread_stats(struct thread_msu_stats* thread_stats)
{
    ...
    for (i = 0; i < thread_stats->array_len; i++) {
        ...
        cur_stat->msu_id = -1;
        cur_stat->queue_item_processed = 0;
        cur_stat->memory_allocated = 0;
        cur_stat->data_queue_size = 0;
    }
    return 0;
}
...
int update_aggregate_stats_size(struct thread_msu_stats* thread_stats, unsigned int total_worker_threads){
    //FIXME LATER for now just free the whole thing and init again since we will do a memcpy
    if(thread_stats->array_len != 0){
        free(thread_stats->msu_stats_data);
    }
    thread_stats->array_len = MAX_MSUS_PER_THREAD * total_worker_threads;
    thread_stats->msu_stats_data = (struct msu_stats_data*)malloc(sizeof(struct msu_stats_data) * thread_stats->array_len);
    if (!thread_stats->msu_stats_data) {
        log_error("Failed to malloc thread_stats array%s", "");
        return -1;
    }
    memset(thread_stats->msu_stats_data,0, sizeof(struct msu_stats_data) * thread_stats->array_len);
    log_debug("Updated aggregate stats size: new space for total msus : %u", thread_stats->array_len);
    return 0;
}
...
static struct index_tracker* add_index_tracking_entry(struct thread_msu_stats *thread_stats, struct msu_stats_data *msu_stats_data_entry){
...
static struct index_tracker* delete_index_tracking_entry(struct index_tracker *msu_stats_index_tracker, int msu_id){
...
void print_thread_stats_entries(struct index_tracker *msu_stats_index_tracker){
    ...
    for(s=msu_stats_index_tracker; s != NULL; s=(struct index_tracker*)(s->hh.next)) {
        log_debug("msu id %d: stat_data->msu_id %d, queue_size: %u", s->id, s->stats_data_ptr->msu_id, s->stats_data_ptr->data_queue_size);
    }
}
...
void iterate_print_thread_stats_array(struct thread_msu_stats* thread_msu_stats){
    ...
    for (i = 0; i < thread_msu_stats->array_len; i++) {
        ...
        log_stats("msu_id: %d, items_prcsd: %u, mem_usage: %u, q_size: %u",
            ...
            thread_msu_stats->msu_stats_data[i].queue_item_processed,
            thread_msu_stats->msu_stats_data[i].memory_allocated,
            thread_msu_stats->msu_stats_data[i].data_queue_size);
    }
}
...
int remove_msu_thread_stats(struct thread_msu_stats* thread_stats, int msu_id){
    ...
    stats_data_entry->queue_item_processed = 0;
    stats_data_entry->memory_allocated = 0;
    stats_data_entry->data_queue_size = 0;
    thread_stats->num_msus--;
    mutex_unlock(thread_stats->mutex);
...
int copy_stats_from_worker(unsigned int all_threads_index){
    ...
    memcpy(&main_msu_stats_data[cpy_start_index], worker_thread->thread_stats->msu_stats_data, sizeof(struct msu_stats_data) * MAX_MSUS_PER_THREAD);
