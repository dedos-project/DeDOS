...
int init_thread_stats(struct thread_msu_stats* thread_stats) {
    ...
    thread_stats->msu_stats_data = (struct msu_stats_data*)malloc(
        sizeof(struct msu_stats_data)* thread_stats->array_len);
    ...
    for (i = 0; i < thread_stats->array_len; i++) {
        ...
        cur_stat->queue_item_processed = malloc(sizeof(struct timed_stat));
        if (!cur_stat->queue_item_processed) {
            log_error("Failed to malloc timed_stat%s","");
            return -1;
        }
        cur_stat->queue_item_processed->value = 0;

        cur_stat->memory_allocated = malloc(sizeof(struct timed_stat));
        if (!cur_stat->memory_allocated) {
            log_error("Failed to malloc timed_stat%s","");
            return -1;
        }
        cur_stat->memory_allocated->value = 0;

        cur_stat->data_queue_size = malloc(sizeof(struct timed_stat));
        if (!cur_stat->data_queue_size) {
            log_error("Failed to malloc timed_stat%s","");
            return -1;
        }
        cur_stat->data_queue_size->value = 0;
    }
    return 0;
}
...
int update_aggregate_stats_size(struct thread_msu_stats* thread_stats,
    ...
    if (thread_stats->array_len != 0) {
        ...
        for (i = 0; i < thread_stats->array_len; i++) {
            cur_stat = &(thread_stats->msu_stats_data[i]);
            free(cur_stat->queue_item_processed);
            free(cur_stat->memory_allocated);
            free(cur_stat->data_queue_size);
        }
...
void print_thread_stats_entries(struct index_tracker *msu_stats_index_tracker) {
    ...
    for (s=msu_stats_index_tracker; s != NULL; s=(struct index_tracker*)(s->hh.next)) {
        log_debug("msu id %d: stat_data->msu_id %d, queue_size: %u",
                  s->id, s->stats_data_ptr->msu_id, s->stats_data_ptr->data_queue_size->value);
    }
}
...
void iterate_print_thread_stats_array(struct thread_msu_stats* thread_msu_stats) {
    ...
    for (i = 0; i < thread_msu_stats->array_len; i++) {
        ...
        log_stats("msu_id: %d, items_prcsd: %u, mem_usage: %u, q_size: %u",
                  ...
                  thread_msu_stats->msu_stats_data[i].queue_item_processed->value,
                  thread_msu_stats->msu_stats_data[i].memory_allocated->value,
                  thread_msu_stats->msu_stats_data[i].data_queue_size->value);
    }
}
...
int register_msu_with_thread_stats(struct thread_msu_stats* thread_stats, int msu_id) {
    ...
    thread_stats->msu_stats_data[empty_index].queue_item_processed =
        malloc(sizeof(struct timed_stat));
    if (!thread_stats->msu_stats_data[empty_index].queue_item_processed) {
        log_error("Failed to malloc timed_stat%s","");
        return -1;
    }
    thread_stats->msu_stats_data[empty_index].queue_item_processed->value = 0;

    thread_stats->msu_stats_data[empty_index].memory_allocated =
        malloc(sizeof(struct timed_stat));
    if (!thread_stats->msu_stats_data[empty_index].memory_allocated) {
        log_error("Failed to malloc timed_stat%s","");
        return -1;
    }
    thread_stats->msu_stats_data[empty_index].memory_allocated->value = 0;

    thread_stats->msu_stats_data[empty_index].data_queue_size =
        malloc(sizeof(struct timed_stat));
    if (!thread_stats->msu_stats_data[empty_index].data_queue_size) {
        log_error("Failed to malloc timed_stat%s","");
        return -1;
    }
    thread_stats->msu_stats_data[empty_index].data_queue_size->value = 0;
...
int remove_msu_thread_stats(struct thread_msu_stats* thread_stats, int msu_id) {
    ...
    free(stats_data_entry->queue_item_processed);
    free(stats_data_entry->memory_allocated);
    free(stats_data_entry->data_queue_size);
    thread_stats->num_msus--;
    mutex_unlock(thread_stats->mutex);
