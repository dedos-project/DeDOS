...
#define PICO_TCP_WAIT_FOR_RESTORE             1000u  /* 1 second */
...
int pico_transport_receive(struct pico_frame *f, uint8_t);
int pico_socket_sendto_transport_offset(struct pico_socket *s);

/* Interface towards TCP Handshake MSU (protocol) */
//int msu_process_hs_request_in(struct pico_protocol *self, struct pico_frame *f);
