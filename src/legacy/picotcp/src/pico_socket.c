...
int socket_cmp(void *ka, void *kb)
{
...
int8_t pico_socket_add(struct pico_socket *s)
{
...
#ifdef DEBUG_SOCKET_TREE
    {
        ...
        log_debug("Current sockets for port %u >>>\n",short_be(s->local_port));
        pico_tree_foreach(index, &sp->socks){
            s = index->keyValue;
...
void socket_garbage_collect(pico_time now, void *arg)
{
...
void hs_socket_garbage_collect(pico_time now, void *arg, void *timers)
{
    ...
    struct pico_frame *f_in = pico_dequeue(&s->q_in);
    struct pico_frame *f_out = pico_dequeue(&s->q_out);
    while(f_in || f_out)
    {
        if(f_in)
        {
            pico_frame_discard(f_in);
            f_in = pico_dequeue(&s->q_in);
        }
    ...
    if(is_sock_tcp(s)){
        ...
        hs_timer_cancel(timers, tcp->retrans_tmr);
        hs_timer_cancel(timers, tcp->keepalive_tmr);
        hs_timer_cancel(timers, tcp->fin_tmr);
...
int8_t pico_socket_del(struct pico_socket *s)
{
...
#ifdef DEBUG_SOCKET_TREE
    {
        struct pico_tree_node *index;
        /* RB_FOREACH(s, socket_tree, &sp->socks) { */
        dbg("Current sockets for port %u >>>\n",short_be(s->local_port));
        pico_tree_foreach(index, &sp->socks){
            s = index->keyValue;
            dbg(">>>> List Socket lc=%hu rm=%hu\n", short_be(s->local_port), short_be(s->remote_port));
        }
...
int pico_socket_sendto_transport_offset(struct pico_socket *s)
{
    int header_offset = -1;
    #ifdef PICO_SUPPORT_TCP
...
#ifndef PICO_SUPPORT_DEDOS_MSUS
...
int pico_transport_process_in(struct pico_protocol *self, struct pico_frame *f)
{
    ...
    /* transport layer discards the frame after processing */
    pico_frame_discard(f);
    return ret;
}
