...
{
    /* dedos
     * ip layer enqueues data to transport layer protocol queues
     */
    int32_t ret = -1;
    switch (proto) {
    ...
    case MSU_PROTO_TCP_HANDSHAKE:
        /* dedos
         * enqueue request for HS here
         * for the reply part when the HS MSU replies back
         * the frame should be enqueued in the TCP queue.
         * Dequeue should take of this particular frame
         * identified by the proto field = MSU_PROTO_TCP_HANDSHAKE
         *
         * Later this might to the data tcp MSU
         */
/* Not used here since routing tables manage this now from runtime
        ret = pico_enqueue(msu_tcp_handshake.q_in, f);
        dbg("Enqueued MSU_PROTO_TCP_HANDSHAKE request: %d\n", ret);
*/
        break;

    case MSU_PROTO_TCP_CONN_RESTORE:
        dbg("pico_transport_receive: MSU_PROTO_TCP_CONN_RESTORE data to be enqueued in tcp in_queue\n");
        ret = pico_enqueue(pico_proto_tcp.q_in, f);
        dbg("Enqueued MSU_PROTO_TCP_CONN_RESTORE request: %d\n", ret);
        break;
#endif
    default:
...
void pico_stack_tick(void)
{
    ...
    //printf("Calling transport loop INPUT direction\n");
    ...
    //printf("Calling transport loop OUTPUT direction\n");
...
MOCKABLE uint32_t pico_timer_add(pico_time expire, void (*timer)(pico_time, void *), void *arg)
{
    ...
    if (Timers->n > PICO_MAX_TIMERS) {
        dbg("PICOTCP: Warning: I have %d timers\n", (int)Timers->n);
    }
...
#ifdef PICO_SUPPORT_DEDOS_MSUS
//    pico_protocol_init(&msu_tcp_handshake);
#endif
