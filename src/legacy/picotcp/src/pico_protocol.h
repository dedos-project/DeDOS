...
struct pico_protocol {
    ...
    void* (*collect_state)(void *data); /* collect and serialize msu specific state to transfer */
    int (*restore)(struct pico_protocol *self, void *buf, uint16_t bufsize); /* deserialize and restore received state */
    int (*same_machine_move_state)(void *data, void *optional_data); /* move state within same process and physical machine, like pointers, runtime should call this */

    /* To add routing table support */
