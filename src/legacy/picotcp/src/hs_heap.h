...
#define HS_DECLARE_HEAP(type, orderby) \
    struct heap_ ## type {   \
        uint32_t size;    \
        uint32_t n;       \
        type *top;        \
    ...
    static inline int hs_heap_insert(struct heap_ ## type *heap, type * el) \
    { \
        uint32_t i; \
        type *newTop; \
        if (++heap->n >= heap->size) {                                                \
    ...
    static inline int hs_heap_peek(struct heap_ ## type *heap, type * first) \
    { \
        type *last;           \
        uint32_t i, child;        \
        if(heap->n == 0) {    \
    ...
    static inline type *hs_heap_first(heap_ ## type * heap)  \
    { \
        if (heap->n == 0)     \
            return NULL;        \
        return &heap->top[1];  \
    } \
    static inline heap_ ## type *hs_heap_init(void) \
    { \
