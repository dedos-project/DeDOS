    ...
    if isinstance(output, dict) or isinstance(output, OrderedDict):
        ...
        for i, v in enumerate(output):
            if isinstance(v, dict) or isinstance(v, OrderedDict):
                ...
                output[i] = str(v)
def make_dfg(yml_filename, pretty=False, i=0):
    ...
    msus_out = []
    for msu in msus:
        msus_out.extend(make_msus_out(msu, i))
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print ("Usage: %s <input.yml> <output.json>" % sys.argv[0])
        exit(1)
    ...
    while True:
        out_file.seek(0)
        out_file.write(make_dfg(sys.argv[1], True, i))
        out_file.truncate()
        i += 1
        time.sleep(2)
